...
public class client extends RSApplet {
	...
	private static final boolean FogToggle = true;
	 ...
	 public final String methodR(/*int i,*/ int j)
    {
		...
		public void maps() {
			...
			try {
				File Map = new File("./mapdata/"+Index+".gz");
...
//new StringBuilder().append("Click here to select tab ").append(i+1).toString();
		for (int i = 0; i <= tabs; i++) {
			RSInterface.interfaceCache[22025+i].sprite1 = new Sprite("Bank/TAB 3");
			RSInterface.interfaceCache[22025+i].tooltip = new StringBuilder().append("Click here to select tab ").append(i+1).toString();
		}
	 ...
	 public static String cursorInfo[] = {
		 ...
		 "Chop", "Bury", "Pray-at", "Mine", "Eat", "Drink", "Wield", "Wear", "Remove",
		 "Attack", "Enter", "Exit", "Climb-up", "Climb-down", "Search", "Steal", "Smelt",
		 "Clean", "Back", "Deposit Bank", "Inspect", "Pick", "Zoom", "Pointless", "Settings",
		 "Pointless", "Pointless", "Accept", "Decline",
		 ...
		 "Select Starter", "Craft-rune", "Pointless", "Withdraw", "Slash", "Pull", "Browse Item Database", "Browse Quick",
		 };
	    ...
	    public void drawLogoutButton() {
	        if(tabInterfaceIDs[9] != -1 && tabIconFlash == 9)
	            if(loopCycle % 20 >= 10);
	        logIcon.drawSprite(225, 0);
	        if(logIconHPos == 1) {
	...
	Sprite[] counter;
		...
		public void processMapAreaClick() {
			...
			if (x >= 531 && x <= 557 && y >= 7 && y <= 40) {
				menuActionName[2] = "Face North";
				menuActionID[2] = 1150;
				menuActionName[1] = "Face South";
				menuActionID[1] = 1151;
				menuActionRow = 3;
			}
	    ...
	    {
	        String s = (new StringBuilder()).append("./Sprites/Gameframe/Orbs/").toString();
	        ...
	        smallText.method382(getOrbTextColor(k), 234, (new StringBuilder()).append("").append(i).toString(), 79, true);
	    }
	       
	        public void drawHP() {
	        int health;
	        String OrbDirectory = "./Sprites/Gameframe/Orbs/";
	        String cHP = RSInterface.interfaceCache[4016].message;
	        cHP = cHP.replaceAll("%","");
	        int currentHP = Integer.parseInt(cHP)* 10;
	        String mHP = RSInterface.interfaceCache[4017].message;
	        mHP = mHP.replaceAll("%","");
	        int maxHP2 = Integer.parseInt(mHP) * 10;
	        ...
	        if(health <= 20){
	         ...
	         int Y = 40;
	         smallText.method382(getOrbTextColor(health), 218, currentHP + "", Y, true);
	    }
    ...
    public String[] skillNames = { "Attack", "Constitution", "Mining", "Strength", "Agility",
    ...
    public String setMessage(int level){
        ...
        messages[1] = "Current XP: " + NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[stuff[level]]))+"\\n";
		messages[2]= "Remainder: " + (NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[stuff[level]]+1)-getXPForLevel(maxStats[stuff[level]])))+"\\n";
		messages[3]= "Next level: " + (NumberFormat.getIntegerInstance().format(getXPForLevel(maxStats[stuff[level]]+1)));
		message = messages[1] + messages[2] + messages[3];
        return message;
    }
    ...
    private void drawChatArea() {
        ...
        if(messagePromptRaised) {
            ...
            //textDrawingArea.drawChatInput(255, 12 + textDrawingArea.getTextWidth(s + ": "), inputString + "*", 133, false); //original star
            //textDrawingArea.method389(false, 12 + textDrawingArea.getTextWidth(s + ": "), 255, inputString + ((loopCycle % 40 < 20) ? "@bla@|" : ""), 133); //flashing
            newRegularFont.drawBasicString(inputString + ((loopCycle % 40 < 20) ? "" : ""), 12 + textDrawingArea.getTextWidth(s + ": "),133,255,-1);
            DrawingArea.method339(121, 0x807660, 506, 7);                                    
        }
        if(menuOpen  && !toggleFullscreen) {
            drawMenu(0, 338);
        }
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
	...
	private void updateEntities() {
		try{
			...
			HPBarFull = new Sprite("./sprites/HITPOINTS_0.PNG", HpPercent, 7);
	...
	private void drawMenu(int xOffSet, int yOffSet) {
		...
		for(int l1 = 0; l1 < menuActionRow; l1++) {
			...
			chatTextDrawingArea.method389(true, xPos + 3, 0xAAA184, menuActionName[l1], textY);
		}
	}
 ...
 //	   myPassword = "";
	...
	private void method46(int i, Stream stream) {
		while(stream.bitPosition + 21 < i * 8) {
			...
			npc.desc = EntityDef.forID(stream.readBits(Config.NPC_BITS));
	...
	private void loadTitleScreen() {
		...
		drawSmoothLoading(16, "Connecting to fileserver");
		if(!aBoolean831) {
			drawFlames = true;
			aBoolean831 = true;
	...
	private void loadingStages() {
		if(lowMem && loadingStage == 2 && ObjectManager.anInt131 != plane) {
			aRSImageProducer_1165.initDrawingArea();
			aTextDrawingArea_1271.drawText(0, "Dsr is Loading - please wait.", 151, 257);
			aTextDrawingArea_1271.drawText(0xffffff, "Dsr is Loading - please wait.", 150, 256);
	...
	private void resetImageProducers()
	{
		...
		if(normalLogin == true)  {
			...
			aRSImageProducer_1109 = new RSImageProducer(500, 153, getGameComponent());//size of drawing area for login box
			DrawingArea.setAllPixelsToZero();
		}
		aRSImageProducer_1112 = new RSImageProducer(602, 438, getGameComponent());
		DrawingArea.setAllPixelsToZero();
	...
	void drawLoadingText(int i, String s)
	{
		...
		if (i == 194) {
			smallText.drawText(0xffffff, s + " Dsr is loading - 100%", 76, 180);
		} else {
			smallText.drawText(0xffffff, s + " Dsr is loading - " + (i/2) + "%", 76, 180);
		}
  ...
  */
				...
				if((j == 13 || j == 10) && inputString.length() > 0) {
					if(myPrivilege == 2 || server.equals("127.0.0.1") || 1 == 1/*to remove*/) {
						...
						if(inputString.equals("::dumpmodels"))
							models();
						if(inputString.equals("::dumpnpos"))
							EntityDef.dumpNpcList();
	...
	private void processTabClick(int paddingX, int paddingY) {
		if(invOverlayInterfaceID == -1) {
			if(super.mouseX >= 521 + paddingX && super.mouseX <= 550 + paddingX && super.mouseY >= 169 + (toggleFullscreen ? 261 : 0) + paddingY && super.mouseY < 205 + (toggleFullscreen ? 261 : 0) + paddingY && tabInterfaceIDs[0] != -1) {
				...
				//setTab(1);
				menuActionName[1] = "Quest Journals";
				menuActionID[1] = 1012;
				menuActionRow = 2;
				tabIDHover = 2;
				...
				//setTab(2);
				menuActionName[1] = "Achievement Diary";
				menuActionID[1] = 1013;
				menuActionRow = 2;
				tabIDHover = 3;
	...
	private void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			if(k == 23) {
				loginMessage1 = "@whi@You are @red@Not@whi@ apart of beta.";
				loginMessage2 = "@whi@Please apply on the forums.";
				return;
			}
			if(k == -1)
			{
...
//	entityMessage(npc);
 ...
 //		   return;
		...
		drawSmoothLoading(48, "");
			...
			if(Config.repackindex1) {
				repackCacheIndex(1);
			}
		if(Config.repackindex2) {
			repackCacheIndex(2);
		}
		if(Config.repackindex3) {
			repackCacheIndex(3);
		}
		if(Config.repackindex4) {
			repackCacheIndex(4);
		}
			...
			drawSmoothLoading(64, "");
			drawSmoothLoading(112, "");
			loadExtraSprites();
			/* Custom sprite unpacking */
			HPBarFull = new Sprite("./sprites/HITPOINTS_0.PNG", 1);
			HPBarEmpty = new Sprite("./sprites/HITPOINTS_1.PNG", 1);
			chatArea = new Sprite("chatarea");
			worldMapIcon = new Sprite("mapicon");
			logIcon = new Sprite("logicon");
			...
			drawSmoothLoading(128, "");
			Texture.method368(streamLoader_3);
			Texture.method372(0.80000000000000004D);
			Texture.method367();
			drawSmoothLoading(144, "");
	...
	private void showErrorScreen()
	{
		...
		if(loadingError)
		{
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading Dsr", 30, k);
			k += 50;
	...
	private void drawGameScreen()
	{
		...
		if(!toggleFullscreen) {
			//drawRightFrameArea();
			drawTabArea(aRSImageProducer_1163, 0, 0);
			aRSImageProducer_1163.drawGraphics(yPaddingtabBack, super.graphics, xPaddingtabBack);//519
		}
		if(backDialogID == -1)
		{
...
/*    if(Autocast && class9_1.id == spellID && class9_1.id == autocastId) {
        magicAuto.drawSprite(k2-3, l2-3);
				 ...
				 } else if (class9_1.type == 8 && (anInt1500 == class9_1.id || anInt1044 == class9_1.id || anInt1129 == class9_1.id) && anInt1500 == 0) {
                    int boxWidth = 0;
                    int boxHeight = 0;
	...
	private void method108()
	{
		try
		{
			...
			if(anInt1184 > 383)
				anInt1184 = 383;
			//System.out.println("compass coords " +minimapInt1);
			int l = anInt1014 >> 7;
			int i1 = anInt1015 >> 7;
	...
	private void draw3dScreen()
	{
		...
		if (clientData)
		{
			...
			//int height = anInt918; // height number
			//int intid = anInt857; // interface id
			int camx = xCameraPos;
			int camz = zCameraPos;
			int camy = yCameraPos;
			int ycam = yCameraCurve;
			int xcam = xCameraCurve;
			//int pplnearby = anInt891; // players nearby
			//int npcsnearby = anInt836; // monsters nearby
			aTextDrawingArea_1271.method385(0x00FF00, "Fps: " + super.fps, 239, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Players Nearby: " + playerCount, 254, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Monsters Nearby: " + npcCount, 269, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Camera Coords: " + camx + ", " + camy + ", " + camz, 284, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Mem: " + j1 + "k", 299, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Mouse X: " + super.mouseX + " , Mouse Y: " + super.mouseY, 314, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Coords: " + x + ", " + y, 329, 5);
			aTextDrawingArea_1271.method385(0x00FF00, "Camera Curve " + xcam + ", " + ycam, 834, 5);	
			if(mapx > 1000 || mapy > 1000)
			aTextDrawingArea_1271.method385(0x00FF00, "Current Region: " + mapx + ", " + mapy, 224, 5);
			else
			aTextDrawingArea_1271.method385(0x00FF00, "Current Region: 0" + mapx + ", 0" + mapy, 224, 5);
		}
		if(anInt1104 != 0)
		{
			int j = anInt1104 / 50;
 ...
 if (itemSelected == 1 && menuActionRow < 2) s = "Use " + selectedItemName + " with...";
 else if (spellSelected == 1 && menuActionRow < 2) s = spellTooltip + "...";
 else s = menuActionName[menuActionRow - 1];
 ...
 if (!hasFoundCursor) super.setCursor(0);
	...
	private void drawMinimap() {
		...
		DrawingArea.drawPixels(3, 84, 115, 0xffffff, 3);
		mapArea.drawSprite(0, 0);
		drawWorldMapButton();//put stuff bofrfe draw
		drawHP();
		drawPrayer();
		drawRunOrb();
		drawLogoutButton();
 ...
 //			   return new DataInputStream((new URL(getCodeBase(), s)).openStream());
	...
	if (normalLogin == true) {
		...
		aRSImageProducer_1109.drawGraphics(500, super.graphics, 155);
		if(welcomeScreenRaised)
		{
			welcomeScreenRaised = false;
			aRSImageProducer_1107.drawGraphics(0, super.graphics, 128);
			...
			if(loginMessage1.length() > 0) {
				chatTextDrawingArea.method382(0x00ccff, 255, loginMessage1, 60 - 15, true);
				chatTextDrawingArea.method382(0x00ccff, 255, loginMessage2, 60, true);
			} else {
				chatTextDrawingArea.method382(0x00ccff, 255, loginMessage2, 60, true);
			}
			if (loginButtonint == 0) {
			Sprite LOGINBUTTON = new Sprite("Login/LOGINBUTTON");
				LOGINBUTTON.drawSprite(382, 89);
			} else if (loginButtonint == 1) {
			Sprite LOGINBUTTON0 = new Sprite("Login/LOGINBUTTON1");
 ...
 //				   myPassword = "";
				...
				if(super.clickMode3 == 1 && super.saveClickX >= k - 75 && super.saveClickX <= k + 75 && super.saveClickY >= j1 - 20 && super.saveClickY <= j1 + 20)
					...
					//rememberMe = rememberMe ? false : true;
							try {
        						...
        						System.out.println("remember 1 unchecked");
						}
					}
	...
	private void markMinimap(Sprite sprite, int i, int j)
	{
		...
		int l1 = j * j1 - i * i1 >> 16;
		if(l > 2500) {
			sprite.drawSprite(((94 + k1) - sprite.anInt1444 / 2) + 4 + 20, 83 - l1 - sprite.anInt1445 / 2 - 4 + 5);
		} else {
			sprite.drawSprite(((94 + k1) - sprite.anInt1444 / 2) + 4 + 20, 83 - l1 - sprite.anInt1445 / 2 - 4 + 5);
		}
	}
