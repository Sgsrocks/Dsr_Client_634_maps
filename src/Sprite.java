...
public final class Sprite extends DrawingArea {
    ...
    /**
     * Run trough all Sprite pixels searching for pixels 100% Transparent(Alpha = 0);
     * Then clear its data.
     * @param a the alpha value necessary to make pixel invisible.
     */
...
public static Image downScaleImage(Image image, int width, int height) {
	if(width > 765 || width > 503)
		...
		return image;
		//upscaling looks pretty bad
}

	public String location = "./Sprites/";

	public Sprite(byte abyte0[], Component component) {		try {
		Image image;
			image = Toolkit.getDefaultToolkit().createImage(location+"Background.png");
		Image resize = downScaleImage(image, 765, 503);
		MediaTracker mediatracker = new MediaTracker(component);
		mediatracker.addImage(resize, 0);
		mediatracker.waitForAll();
		myWidth = resize.getWidth(component);
		System.out.println("my width: " + myWidth);
		myHeight = resize.getHeight(component);
	...
	public Sprite(String img, Component component) {		try {
		Image image;
			image = Toolkit.getDefaultToolkit().createImage(location+"Background.png");
Image resize = downScaleImage(image, 765, 503);
	...
	setTransparency(250, 220, 56);      
	 ...
	 public Sprite(String img) {
			...
			if (img.equalsIgnoreCase("chatarea")) {
				 ...
				 else if (img.equalsIgnoreCase("tabhover") || img.equalsIgnoreCase("tabclicked")) {
		                myWidth = 32;
		                myHeight = 37;
		            }
		            for(int i = 0; i <= 14; i++) {
    ...
    /**
     * Draw a sprite using ARGB values.
     * 
     * @param posX: X position of the sprite.
     * @param posY: Y position of the sprite.
     */
    public void drawAdvancedSprite(int posX, int posY) {
        ...
        if (posX < DrawingArea.topX) {
            int widthLeft = DrawingArea.topX - posX;
            width -= widthLeft;
        ...
        if (posX + width > DrawingArea.bottomX) {
            int widthLeft = (posX + width) - DrawingArea.bottomX;
            width -= widthLeft;
            spriteScanSize += widthLeft;
            containerScanSize += widthLeft;
        }
        ...
        if (width > 0 && height > 0) {
            drawToContainer(width, height, pixelOffset, myPixels, spriteScanSize, containerPixel, DrawingArea.pixels, containerScanSize);
        }
        setAlphaTransparency(0);
    }
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                if (argb != 0) {
                    ...
                    containerData[containerPixel++] = ((argb & 0xff00ff) * alpha + (originalColor & 0xff00ff) * transparency & 0xff00ff00) + ((argb & 0xff00) * alpha + (originalColor & 0xff00) * transparency & 0xff0000) >> 8;
                } else {
                    containerPixel++;
                }
            }
	...
	public void drawSprite1(int i, int j, int var1)
	{
		i += anInt1442;
		j += anInt1443;
		...
		if(!(l1 <= 0 || k1 <= 0))
		{
			method351(j1, l1, DrawingArea.pixels, myPixels, j2, k1, i2, var1, i1);
		}
	}                                                                                                             
